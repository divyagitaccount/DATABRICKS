...
@dp.materialized_view(
    name="gold_staff_geo_enriched_dlt",
    comment="Staff enriched with Geo Location data",
    table_properties={"quality": "gold"}
)
def gold_staff_geo_enriched_dlt2():
    # Read the upstream silver tables
    df_staff = spark.read.table("silver_staff_dlt")
    df_geo = spark.read.table("silver_geotag_dlt")
    
    return (
        df_staff.join(
            df_geo, 
            df_staff.origin_hub_city == df_geo.city_name, 
            "inner"
        )
        .select(
            df_staff["*"],
...
def gold_shipment_stats2():
    return (
        spark.read.table("silver_shipments_dlt")
        .groupBy("source_city")
        .agg(
